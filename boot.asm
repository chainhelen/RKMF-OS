...
LABEL_SEARCH_IN_ROOT_DIR_BEGIN:
	...
	mov	bx,	OffsetOfLoader	;	bx	<-	OffsetOfLoader 于是，es:bx = BaseOfLoader
	mov	ax,	[wSectorNo]		;	ax <- Root Directory 中的某Sector	号
	mov	cl,	1
	call	ReadSector

	mov		si, LoaderFileName	;	ds:si -> "Loader bin"
	mov		di, OffsetOfLoader	;	es:di -> BaseOfLoader * 10h + OffsetOfLoader
	cld
	mov		dx, 10h
...
LABEL_FILENAME_FOUND:
	...
	add		cx,	ax
	add		cx,	DeltaSectorNo	;	cl <- Loader.bin 的其实扇区号 (0 - based)
	mov		ax,	BaseOfLoader
	mov		es,	ax				;	es <- BaseOfLoader
